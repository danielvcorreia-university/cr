...
begin
    ...
    begin
        case s_seg_activate_count is
            when "000"  => an_L <= x"FE"; s_led_bcd <= en_L(0) & hex0; dp_L <= en_dp_L(0);
            when "001"  => an_L <= x"FD"; s_led_bcd <= en_L(1) & hex1; dp_L <= en_dp_L(1);
            when "010"  => an_L <= x"FB"; s_led_bcd <= en_L(2) & hex2; dp_L <= en_dp_L(2);
            when "011"  => an_L <= x"F7"; s_led_bcd <= en_L(3) & hex3; dp_L <= en_dp_L(3);
            when "100"  => an_L <= x"EF"; s_led_bcd <= en_L(4) & hex4; dp_L <= en_dp_L(4);
            when "101"  => an_L <= x"DF"; s_led_bcd <= en_L(5) & hex5; dp_L <= en_dp_L(5);
            when "110"  => an_L <= x"BF"; s_led_bcd <= en_L(6) & hex6; dp_L <= en_dp_L(6);
            when "111"  => an_L <= x"7F"; s_led_bcd <= en_L(7) & hex7; dp_L <= en_dp_L(7);
